...
const DUMMY_PASSWORD_HASH = '$2b$10$C6UzMDM.H6dfI/f/IKcEeO6E7dYkP82LPgAvFp8ZUBKbjDg7sWXBK';

const safeCompareText = (left, right) => {
...
export const handleLoginAttempt = async (ip, username, password) => {
  ...
  if ((!storedPassword && !storedPasswordHash) || !storedUser) {
    console.error("ADMIN_USERNAME and either ADMIN_PASSWORD or ADMIN_PASSWORD_HASH must be set.");
  ...
  const usernameMatch = safeCompareText(providedUsername, storedUser);
  const passwordHashToCheck = storedPasswordHash || DUMMY_PASSWORD_HASH;
  const passwordMatch = storedPasswordHash
    ? await bcrypt.compare(providedPassword, passwordHashToCheck)
    : safeCompareText(providedPassword, storedPassword);
